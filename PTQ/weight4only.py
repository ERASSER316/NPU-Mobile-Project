...
device = "cuda"
save_dir = "../model_quantization/llama-3.2-1b-int4woq-ptq"
#加载分词器
...
#复制一个新的模型用于PTQ，防止改变原始模型的权重
ptq_model = copy.deepcopy(model).to(device)

from torchao.quantization import Int4WeightOnlyConfig, quantize_
woq_config = Int4WeightOnlyConfig(
    group_size=32,     # 推荐：32/64，先用32做 baseline
    # 其他用默认即可：uint4、per-group、非对称、tensor_core_tiled 布局等
)
quantize_(ptq_model, woq_config)#得到权重INT4量化后的模型
#保存量化信息到config文件中
